    ...
    def __init__(self, input_f, output_f, pin_data, grid_dimensions):
        ...
        for g, sz in output_f.items():
            if g == "bounding_box":
                ...
                self.canvas.create_text(x1 + (x2 - x1) / 2, y1 + (y2 - y1) / 2, font=("Arial", int(ALPHA*grid_size)), text=g)
                ...
                if g in pin_data:
                    for pin in pin_data[g]:
                        ...
                        self.canvas.create_oval(px - PIN_LAMBDA, py - PIN_LAMBDA, px + PIN_LAMBDA, py + PIN_LAMBDA, fill="black")  # Draw bold pin point
